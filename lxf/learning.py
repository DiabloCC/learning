...
def check_version():
    ...
    if v .major == 3 and v.minor >= 4:
        return True
    print('Your current python is %d.%d. Please use Python 3.4 or above.' % (v.major,v. minor))
    return False
...
def application(environ, start_response):
    ...
    if method == 'GET' and path == '/env':
        ...
        L.append(b'</html>')
        return L
    if host != HOST or methos != 'POST' or path != '/run' or not environ.get('CONTENT_TYPE', '').lower().startswith('application/x-www-form-urlencoded'):
